...
with DAG(
    dag_id="load_aggregated_data_to_s3_company2",
    schedule_interval=None,
    start_date=datetime(2022, 1, 1),
    schedule=[
        Dataset("snowflake://crx_data_bank2"),
    ],
    catchup=False,
) as dag:
    ...
    export_data_s3 = aql.export_file(
        task_id="save_file_to_s3_company2",
        input_data=crx_data_table,
        output_file=File(
            path=f"{S3_BUCKET_NAME}/crx_data_company2.csv",
            conn_id=AWS_CONN_ID,
        ),
        if_exists="replace",
        outlets=[Dataset("databricks://prediction_input_company2")]
    )