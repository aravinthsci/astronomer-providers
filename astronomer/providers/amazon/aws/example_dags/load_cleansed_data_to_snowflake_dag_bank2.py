...
with DAG(
        dag_id="load_cleansed_data_to_snowflake_bank2",
        schedule_interval=None,
        start_date=datetime(2022, 1, 1),
        schedule=[
            Dataset('s3://ingestionfeeds/crxbank2.csv'),
        ],
        catchup=False,
) as dag:
    crx_data_table = Table(conn_id=SNOWFLAKE_CONN, name="temp_crx_data_bank2", metadata=Metadata(
        schema=SNOWFLAKE_SCHEMA_NAME, ))

    load_table_with_data = aql.load_file(
        input_file=File(path=f"{S3_BUCKET_NAME_SOURCE}/crxbank2.csv",
                        filetype=FileType.CSV, conn_id=AWS_CONN_ID),
        task_id="load_csv_data_bank2",
        output_table=crx_data_table,
        outlets=[Dataset("snowflake://crx_data_bank2")]
    )
